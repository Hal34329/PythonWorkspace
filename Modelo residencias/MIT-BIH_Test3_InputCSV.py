...
loadAndSavePath = 'C:/Users/HP/Documents/2024/Itesco/ECG_Sim/12-Lead/Simulation_Arr_2'
...
data = pd.read_csv(f"{loadAndSavePath}/ecg_data_testII_V5.csv")

# Lista para almacenar los resultados
resultados = []

# Iterar sobre las filas y realizar la predicción
for index, row in data.iloc[1:].iterrows():
    row_df = pd.DataFrame([row.values], columns=data.columns)
    row_scaled = loaded_scaler.transform(row_df)
    prediccion = loaded_model.predict(row_scaled)
    resultados.append({"Latido": index, "Prediccion": prediccion[0]}) # Agregar al archivo para el JSON
    
    # Imprimir el resultado (sin modificar la salida del modelo)
    print(f"Latido {index}: {prediccion[0]}")

resultados_df = pd.DataFrame(resultados)
resultados_df.to_csv(f"{loadAndSavePath}/Resultados_predicciones.csv", index=False)

with open(f"{loadAndSavePath}/Resultados_predicciones.json", 'w') as f:
    json.dump(resultados, f, indent=4)  # indent=4 para formato legible
